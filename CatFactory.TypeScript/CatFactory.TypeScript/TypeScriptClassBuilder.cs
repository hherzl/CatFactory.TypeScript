...
namespace CatFactory.TypeScript
{
    public class TypeScriptClassBuilder : TypeScriptCodeBuilder
    {
        ...
        public override void Translating()
        {
            ...
            classDeclaration.Add(ObjectDefinition.Export ? "export" : string.Empty);
            classDeclaration.Add("class");
            classDeclaration.Add(ObjectDefinition.Name);
            

            //Lines.Add(new CodeLine("{0}{1}class {2}", Indent(start), , );
            ...
            if (ObjectDefinition.Methods.Count > 0)
            {
                ...
                for (var i = 0; i < ObjectDefinition.Methods.Count; i++)
                {
                    ...
                    Lines.Add(new CodeLine("{0}{1} {2}({3}): {4} {5}", Indent(start + 1), method.AccessModifier.ToString().ToLower(), method.Name, method.Parameters.Count == 0 ? string.Empty : string.Join(", ", method.Parameters.Select(item => string.Format("{0}: {1}", item.Name, item.Type))), method.Type, "{"));
